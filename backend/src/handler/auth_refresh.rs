...
use crate::{AppState, error::HttpError, utils::refresh as refresh_utils, utils::token as jwt_utils};
...
pub async fn refresh_handler(
    ...
    response
        .headers_mut()
        .append(header::SET_COOKIE, refresh_id_cookie.to_string().parse().unwrap());
...
pub async fn logout_handler(
    ...
    let mut response = (StatusCode::OK, Json(json!({"status":"success","message":"logged out"}))).into_response();
    response
        .headers_mut()
        .append(header::SET_COOKIE, clear_cookie.to_string().parse().unwrap());
    response
        .headers_mut()
        .append(header::SET_COOKIE, clear_refresh.to_string().parse().unwrap());
    response
        .headers_mut()
        .append(header::SET_COOKIE, clear_refresh_id.to_string().parse().unwrap());
